...
**所以，对于 `url-loader` 最合理的使用方法是在配置 webpack 时使用 limit 属性，当图片小于 limit 时，就按 `url-loader` 将图片转换为 base64 字符串的方式进行加载，如果大于 limit 就按 `file-loader` 将图片的 url 赋值给 img 的 src 属性的方式进行加载，但是这时候请求图片时需要发送 http 请求。**
...
需要安装 `sass-loader` 和 `node-sass` 这两个 loader。（sass-loader 依赖于 node-sass 所以需要先安装 node-sass）
...
从下往上，先是利用 postcss-loader 进行打包，紧接着是 sass-loader，依次向上。
...
webpack.config.js 的配置和上面是一样的，表明开启了热更新。
...
最佳的实践方式是在使用 git 将代码提交到仓库时进行 ESLint 代码规范的检查，如果代码不规范就不能提交到远程的仓库。其实最好的方式是在 commit 的时候进行 ESLint 检查，如果检查不通过 commit 就会失败。
